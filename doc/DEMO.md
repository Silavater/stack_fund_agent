...
| URL | what it is |
|---|---|
| `/pricing` | 3 plans (Watch free / **Pro NT$299** / Desk NT$999) → Stripe Checkout |
| `/success` | server-verifies the Checkout session is **paid** → unlock |
| `/` | chat with the Hermes agent (runs the skills + engine under `agent/SOUL.md`) |
| `/finops` | the agent's books: P&L + the spend **VoI gate** + the receipts ledger |
...
1. **Buy from 0** — `/pricing` → click **Pro NT$299** → Stripe-hosted Checkout → pay with
...
4. **The business** — open **`/finops`** (or the header link): revenue NT$299 (customer paid),
   cost NT$120 (the system paid its own tools), margin NT$179, and the **VoI gate** with the
   crowd wire **cut** + a **REFUSED** NT$999 over-cap spend (no Stripe call).
...
| 1 | `/pricing` → Pro → Stripe Checkout → `4242…` → `/success` | `http://localhost:5757/pricing` | "A stranger subscribes to the Pro report — NT$299, real Stripe in test mode. No human on our side." | 22 |
...
|---|---|---|---|
| **earn** | customer → StackFund | subscription revenue NT$299 | `/pricing` purchase, `/finops` |
